#-------------------------------------------------------------------
# FileName: B2B_CF_Person.R
# INPUT: The User Business Review Score CSV file
# OUTPUT: Two graphs giving 
# Use: To perform Item-Item Collaborative Filtering using Pearson Similarity
#      We have used Pearson Similarity with Weighted prediction
...
for(topK in 1:25){
  ...
  # 
  # AbsError <- sum(abs(finalResult - Test))/nrow(Test)
  # 
  # sqrt(sum((finalResult - Test)^2))/nrow(Test)
}
...
ggsave(filename = 'MAE B2B CF Pearson.png'
       ,plot = P
       ,width = 8
       ,height = 6)
...
ggsave(filename = 'RMSE B2B CF Pearson.png'
       ,plot = P
       ,width = 8
       ,height = 6)